...
### iOS App Creation (Zero-Shot)
```
use auto-orchestrator: GO create a recipe app where users can save and organize recipes

Pipeline:
1. ANALYZE → intent: create_app, domain: ios, complexity: moderate
2. ENHANCE → extracts features (recipes, ingredients, categories, search, favorites)
3. ROUTE → ios-macos-app-creator + swiftui-architect + ios-testing-specialist
4. EXECUTE → generates complete Xcode project with SwiftUI views, models, tests
```

### iOS App to TestFlight
```
use auto-orchestrator: GO build a todo app and publish it to TestFlight

Pipeline:
1. ANALYZE → intent: create_app + deploy_ios, domain: ios, complexity: moderate
2. ENHANCE → todo features + TestFlight requirements (signing, provisioning)
3. ROUTE → sequential: ios-macos-app-creator → ios-testing-specialist → testflight-publisher
4. EXECUTE → create app → run tests → archive → upload → distribute
```

### macOS Menu Bar App
```
use auto-orchestrator: GO create a macOS menu bar app for quick note taking

Pipeline:
1. ANALYZE → intent: create_app, domain: macos, complexity: simple
2. ENHANCE → menu bar specific requirements, NSStatusItem, popover
3. ROUTE → ios-macos-app-creator with macOS configuration
4. EXECUTE → generates macOS project with MenuBarExtra and SwiftUI popover
```
...
IF is_ios_app_creation(prompt):
    → Route to ios-macos-app-creator as primary
    → Include swiftui-architect for complex UI
    → Include ios-testing-specialist for test generation

IF is_testflight_publish(prompt):
    → Route to testflight-publisher
    → Ensure ios-testing-specialist runs first
    → Sequential execution: create → test → publish

IF is_macos_app(prompt) AND is_menu_bar_app(prompt):
    → Route to ios-macos-app-creator with macOS config
    → Include specific menu bar patterns
```
...
- iOS App Specialists: ios-macos-app-creator, testflight-publisher, swiftui-architect
