...
| Agent | Specialty | When to Use |
|-------|-----------|-------------|
| `ios-macos-app-creator` | End-to-end app creation | Zero-shot app from idea, full app generation |
| `testflight-publisher` | TestFlight distribution | Build, sign, upload, distribute apps |
| `swiftui-architect` | SwiftUI development | iOS/macOS UI components |
...
| `create_app` | ios-macos-app-creator | swiftui-architect, ios-testing-specialist |
| `publish_app` | testflight-publisher | ios-macos-app-creator |
...
| `deploy_ios` | testflight-publisher | ios-testing-specialist |
...
def route(analysis, enhanced_prompt):
    # Step 0: Check for iOS/macOS app creation patterns
    if is_app_creation_request(analysis):
        return route_app_creation(analysis, enhanced_prompt)
...
def is_app_creation_request(analysis):
    """Detect iOS/macOS app creation requests"""
    app_keywords = ['ios app', 'macos app', 'iphone app', 'ipad app', 'mac app',
                    'swift app', 'swiftui app', 'apple app', 'mobile app',
                    'visionos app', 'watchos app', 'testflight', 'app store']
    creation_intents = ['create', 'build', 'make', 'develop', 'generate']

    prompt_lower = analysis.original_prompt.lower()
    has_app_keyword = any(kw in prompt_lower for kw in app_keywords)
    has_creation_intent = any(intent in prompt_lower for intent in creation_intents)

    return has_app_keyword or (analysis.domains[0] == 'ios' and has_creation_intent)

def route_app_creation(analysis, enhanced_prompt):
    """Route iOS/macOS app creation requests"""
    prompt_lower = analysis.original_prompt.lower()

    # Check if publishing is requested
    wants_testflight = any(kw in prompt_lower for kw in
                          ['testflight', 'publish', 'distribute', 'release', 'deploy'])

    if wants_testflight:
        # Full pipeline: create + publish
        return execute_sequential(
            primary='ios-macos-app-creator',
            support=['ios-testing-specialist', 'testflight-publisher'],
            enhanced_prompt=enhanced_prompt,
            workflow='idea_to_testflight'
        )
    else:
        # Just creation
        return execute_coordinated(
            primary='ios-macos-app-creator',
            support=['swiftui-architect', 'ios-testing-specialist'],
            enhanced_prompt=enhanced_prompt,
            workflow='zero_shot_app'
        )
```
...
### iOS/macOS App Creation
For app creation requests:
1. **Zero-shot app requests** → `ios-macos-app-creator` as primary
2. **Complex UI needs** → Add `swiftui-architect` as support
3. **Performance-critical** → Add `ios-performance-engineer`
4. **TestFlight publishing** → Sequential: create → test → `testflight-publisher`
5. **Full pipeline** → Coordinated: all iOS agents

#### App Creation Routing Matrix
| Request Pattern | Primary Agent | Support Agents | Workflow |
|-----------------|---------------|----------------|----------|
| "Create [type] app" | ios-macos-app-creator | swiftui-architect | zero_shot_app |
| "Build iOS app and publish" | ios-macos-app-creator | testflight-publisher | idea_to_testflight |
| "Make macOS menu bar app" | ios-macos-app-creator | swiftui-architect | macos_menubar |
| "Publish to TestFlight" | testflight-publisher | - | testflight_publish |
| "Update app and release" | ios-macos-app-creator | testflight-publisher | update_and_release |
| "Create visionOS app" | ios-macos-app-creator | swiftui-architect | visionos_app |

#### App Keyword Detection
```python
APP_CREATION_KEYWORDS = [
    # Platform keywords
    'ios app', 'iphone app', 'ipad app', 'macos app', 'mac app',
    'visionos app', 'watchos app', 'apple app', 'swift app', 'swiftui',

    # App type keywords
    'mobile app', 'native app', 'universal app',

    # Feature keywords implying app creation
    'create app', 'build app', 'make app', 'develop app', 'generate app',

    # Publishing keywords
    'testflight', 'app store', 'publish app', 'release app', 'distribute app'
]
```
